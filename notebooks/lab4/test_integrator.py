#%%
# add many print statements
import context
from numlabs.lab4.lab4_functions import derivsinter41, initinter41,eulerinter41,midpointinter41
import numpy as np
...
for i in np.arange(1,nsteps):
    print (f'Derivs at y={y} and time={timeVec[i-1]}, are {derivsinter41(coeff, y, timeVec[i-1])}')
    print (f'cal derivs at same time are {-y + timeVec[i-1] + 1}')
    ynew=eulerinter41(coeff,y,timeVec[i-1])
    print (f'After euler step new y is {ynew}')
    print (f'calculated euler is {y + coeff.dt*(-y + timeVec[i-1] + 1)}')
    ye.append(ynew)
